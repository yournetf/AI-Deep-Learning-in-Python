...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    num_elements = 1
    dimension_list = x.shape
    for dimension in dimension_list:
        num_elements = num_elements * dimension
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    multiples = list(range(first_multiple, stop + 1, 10))
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    indices = torch.tensor(x, dtype=torch.long)
    y[torch.arange(N), indices] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    row_mins = x.min(dim=1, keepdim=True).values
    y[y == row_mins] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    sigma = torch.sqrt( ( (x-mu)**2 ).sum(dim=0, keepdim=True) / M )
    y = (x-mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    csum = torch.cumsum(concatenated, dim=0)
    sum_vals = cumsum[ls.cumsum(0) -1]
    sum_vals[1:] -= cumsum[ls.cumsum(0)[:-1] - 1]
    y = sum_vals / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    uniques, indices = torch.unique(x, return_inverse=False, return_counts=False, return_indices=True)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
