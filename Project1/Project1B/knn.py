...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_sq = torch.sum(x_test_flat ** 2, dim=1, keepdim=True).T
    
    cross_term = 2 * torch.matmul(x_train_flat, x_test_flat.T)
    
    dists = x_train_sq + x_test_sq - cross_term
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        _, neighbor_indices = torch.topk(dists[:, j], k=k, largest=False)
...
def knn_cross_validate(
    ...
    for fold in range(num_folds):
        ...
        for k in k_choices:
            ...
            accuracy = (y_pred == y_valid).float().mean().item()
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    k_to_mean_accuracy = {k: torch.tensor(acc_list).mean().item() for k, acc_list in k_to_accuracies.items()}
    
    best_k = min(k_to_mean_accuracy.keys(), key=lambda k: (-k_to_mean_accuracy[k], k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
